...
Client-review mutations (`src/actions/{review-links,review-comments,revisions,review}.ts`, `src/data-access/{review-links,review-comments,change-requests,revisions,approvals}.ts`) follow every pattern documented above unchanged, with one structural addition: **two authorization boundaries now exist side by side.** Creator-side actions still derive identity from `requireAuthenticatedUser()`/`requireOwnedWorkspace()` exactly as before. Client-side actions (`src/actions/review.ts`) instead call `authorizeReviewToken(rawToken)` — a completely independent trust path (see `REVIEW_TOKEN_SECURITY.md`) — with the raw token passed as a **hidden form field**, never a URL param the action itself reads, and re-validated on every single submission rather than once per page load.

`recordActivity`'s `actorType` was widened from a fixed `"CREATOR"` literal to `"CREATOR" | "CLIENT" | "SYSTEM"`; `creatorId` became optional on the same input type (client-triggered rows still set it, resolved from the workspace, so they remain visible in the creator's own audit trail — see `ActivityLog` in `prisma/schema.prisma`).

Status-transition logic that used to be an ad hoc check inside `cancelOwnedWorkspace` is now centralized in `src/lib/workspace-transitions.ts`, and every Phase 6 status-changing function (`createReviewLink`'s DRAFT→IN_REVIEW, `createChangeRequest`'s IN_REVIEW→CHANGES_REQUESTED, `submitRevision`'s CHANGES_REQUESTED→IN_REVIEW, `approveWorkspace`'s IN_REVIEW→APPROVED) routes through the same `assertWorkspaceTransition()` — one allow-list, directly unit-tested against the brief's full permit/forbid matrix.

## Phase 7 addendum

Payment/delivery mutations (`src/actions/payments.ts`, `src/data-access/{payment-orders,payment-verification,payment-finalization,payment-reconciliation,webhook-processing,delivery-retry,download-auth,downloads}.ts`) extend the same allow-list (`APPROVED→PAYMENT_PENDING→PAID→FILES_UNLOCKED→DELIVERED`, see `PAYMENT_ARCHITECTURE.md`) but introduce **three** authorization boundaries rather than two: creator sessions (unchanged), review tokens (client checkout/order creation — same `authorizeReviewToken` trust path as Phase 6), and a brand-new, entirely independent **download-grant token space** (`authorizeDownloadGrant`, see `SECURE_DOWNLOAD_ARCHITECTURE.md`) for original-file access — never reusable as, or interchangeable with, a review token. Two Server Actions exist for the creator side (`retryDeliveryPreparationAction`, `refreshPaymentStatusAction`) following the exact same `useActionState` + `{ error | success }` shape as every prior phase; every payment/download *route handler*, by contrast, follows the Phase 5 precedent of being a real HTTP request/response endpoint (`src/lib/api-errors.ts`'s `apiErrorResponse`) rather than a Server Action, since Checkout, webhooks, and signed-redirect downloads are all genuine request/response workflows, not form submissions.
