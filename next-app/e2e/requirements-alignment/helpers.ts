...
export async function createWorkspaceViaWizard(
  ...
  // In the simplified wizard, we must upload a file to proceed.
  // If no files are provided, we'll upload a tiny 1px dummy image just to get past the wizard.
  // Tests that need specific files will still upload them later using uploadFileAndWaitReady.
  ...
  // Wait for the "Continue to Confirmation" button to become enabled and click it
  // (It only becomes enabled when the files finish processing)
