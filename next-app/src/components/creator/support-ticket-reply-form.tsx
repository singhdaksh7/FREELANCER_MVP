...
import { addCreatorSupportMessageAction, type SupportTicketMessageFormState } from "@/actions/support-tickets";

const INITIAL_STATE: SupportTicketMessageFormState = {};

export function SupportTicketReplyForm({ ticketId }: { ticketId: string }) {
  const [state, action, pending] = useActionState(addCreatorSupportMessageAction, INITIAL_STATE);
